import { ISellerGig } from '@ductan2/sw-shared/src';
...
const gigSchema: Schema = new Schema(
    ...
    },
    {
        ...
        toJSON: {
            transform(_doc, rec) {
                rec.id = rec._id;
                delete rec._id; // remove _id from the response
                return rec;
            }
        }
    }
...
// can be used to get the id of a gig 
/*
const gig = new Gig({ _id: 'some_unique_id' });
console.log(gig.id); // Kết quả sẽ là 'some_unique_id'
console.log(gig._id); // Kết quả cũng sẽ là 'some_unique_id'
 */
